...
NodeName={{ include "common.names.fullname" . }}-slurmd-cpu-[0-` + fmt.Sprintf("%d", r.Spec.Values.SlurmdCPU.ReplicaCount) + `] CPUs=` + fmt.Sprintf("%d", r.Spec.Values.SlurmdCPU.Resources.Requests.Core) + ` CoresPerSocket=` + fmt.Sprintf("%d", r.Spec.Values.SlurmdCPU.Resources.Requests.Core) + ` ThreadsPerCore=1 RealMemory=` + fmt.Sprintf("%d", memory_utils.parse_ram_str(r.Spec.Values.SlurmdCPU.Resources.Requests.Memory)) + ` Procs=1 State=UNKNOWN
NodeName={{ include "common.names.fullname" . }}-slurmd-gpu-[0-` + fmt.Sprintf("%d", r.Spec.Values.SlurmdGPU.ReplicaCount) + `] CPUs=` + fmt.Sprintf("%d", r.Spec.Values.SlurmdGPU.Resources.Requests.Core) + ` CoresPerSocket=` + fmt.Sprintf("%d", r.Spec.Values.SlurmdGPU.Resources.Requests.Core) + ` ThreadsPerCore=1 RealMemory=` + fmt.Sprintf("%d", memory_utils.parse_ram_str(r.Spec.Values.SlurmdGPU.Resources.Requests.Memory)) + ` Procs=1 State=UNKNOWN
